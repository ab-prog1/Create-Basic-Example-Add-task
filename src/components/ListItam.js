     ...
     return (
          <li className={`list-group-item d-flex justify-content-between ${done ? 'bg-success text-white' : ''}`}>
               {done ? <del>{title}</del> : title}
                      <div>
                           <button className="btn btn-small bg-danger text-white" onClick={()=> delHandler(id)}> <i className="fas fa-trash"></i></button>
                           <button className={`btn btn-small text-white  ms-2 ${done ? "bg-warning" : " bg-success"}`} onClick={() => doneHandler(id)}>
                               {
